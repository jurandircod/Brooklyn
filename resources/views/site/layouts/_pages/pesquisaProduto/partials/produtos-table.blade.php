            @foreach ($produtos as $index => $produto)
                <article class="group relative bg-white flex flex-col {{ !$produto->estoque ? 'opacity-60' : '' }}">
                    ...
                    <div class="relative aspect-square overflow-hidden bg-gray-50 mb-4">
                        <a href="{{ route('site.produto', ['id' => $produto->id]) }}" class="block w-full h-full">
                        ...
                        <div
                            ...
                            <button
                                class="w-9 h-9 bg-white border border-gray-200 flex items-center justify-center hover:border-[#5A1F2D] hover:bg-[#5A1F2D] hover:text-white transition-all duration-300 {{ !$produto->estoque ? 'opacity-50 cursor-not-allowed' : '' }}"
                                data-id="{{ $produto->id }}" title="Adicionar ao carrinho"
                    ...
                    <div class="flex-1 flex flex-col space-y-2">
                        ...
                        <!-- Product Name -->
                        <a href="{{ route('site.produto', ['id' => $produto->id]) }}" class="block">
                            <h3
                                class="text-sm font-normal text-gray-900 line-clamp-2 group-hover:text-[#5A1F2D] transition-colors duration-200">
                                {{ $produto->nome }}
                            </h3>
                        </a>
                        ...
                        <div class="pt-2 space-y-2">
                            ...
                            <div class="flex items-center gap-1">
                                @php
                                    $rating =
                                        $produto->avaliacao->count() > 0 ? $produto->avaliacao->first()->estrela : 5;
                                @endphp
                                @for ($i = 1; $i <= 5; $i++)
                                    @if ($i <= $rating)
                        ...
                        @if ($produto->estoque)
                            <button
                                class="mt-3 w-full py-2.5 bg-[#5A1F2D] text-white text-sm font-medium hover:bg-[#7A2F3D] transition-colors duration-300 flex items-center justify-center gap-2"
                                data-id="{{ $produto->id }}">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                        d="M16 11V7a4 4 0 00-8 0v4M5 9h14l1 12H4L5 9z" />
                        ...
                        @else
                            <button
                                class="mt-3 w-full py-2.5 bg-gray-100 text-gray-400 text-sm font-medium cursor-not-allowed flex items-center justify-center gap-2"
                                disabled>
                                Indisponível
                            </button>
                        @endif
